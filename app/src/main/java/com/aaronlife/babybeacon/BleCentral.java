...
public class BleCentral
{
    ...
    public void start()
    {
        ...
        //Log.i("brad:deviceName","devicename"+deviceName);
    }
    ...
    private ScanCallback scanCallback = new ScanCallback()
    {
        ...
        public void onScanResult(int callbackType, ScanResult result)
        {
            ...
            Log.i("brad:Name",device.getName().toString());
            Log.i("brad:Addr.",device.getAddress().toString());
            Log.i("brad:BondState", String.valueOf(device.getBondState()));
            Log.i("brad:type",String.valueOf(device.getType()));
            ...
            if(device.getName().equals("MCF-0000-0000-000") && !isBusy)
            {
                isBusy = true;

                // 建立GATT連線
                bluetoothGatt = device.connectGatt(context, false, gattCallback);
                //Log.i("Class:",bluetoothGatt.getClass().toString());
                Log.i("brad:service","Service"+bluetoothGatt.getServices().toString());
                Log.i("brad","here");
            }
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status)
        {
            ...
            for(BluetoothGattService s : services)
            {
                if(s.getUuid().compareTo(UUID.fromString(BleDefine.CUSTOM_SERVICE)) == 0)
                {
                    ...
                    {
                        if(c.getUuid()
                            .compareTo(UUID.fromString(BleDefine.DEVICE_NAME)) == 0)
                        {
